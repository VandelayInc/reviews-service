...
// ======== FOR MICROSERVICE DEPLOYED DEPLOYED DEPLOYED ======= //
// split even between ratings and reviews
...
const writer = (n = 1) => {
  ...
  while (isReady && n < 3e6 + 1) {
    var randNum = Math.floor((Math.random() * 1e7) + 1);
    if (n % 2 === 0) {
      var url = `GET http://52.53.196.86:3000/rooms/${randNum}/ratings/`;
      if (n !== 3e6) {
        isReady = stream.write(`${url}\n`);
      } else {
        isReady = stream.write(`${url}`);
      ...
      var url = `GET http://52.53.196.86:3000/rooms/${randNum}/reviews/`;
      if (n !== 3e6) {
        isReady = stream.write(`${url}\n`);
      } else {
        isReady = stream.write(`${url}`);
...
// ======= for the PROXY SERVER DEPLOYED DEPLOYED DEPLOYED!!!! =======//
